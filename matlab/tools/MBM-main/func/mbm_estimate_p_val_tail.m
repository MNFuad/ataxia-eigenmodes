...
corr_mat1out = false;
if isnan(observe) & sum(isnan(null)) == length(null) % observe is NaN and all null is NaN, which mean the everyone in the two groups are identical.
        pValueTail = 1;
        rev = true; %no tail
elseif max(null) <= observe
